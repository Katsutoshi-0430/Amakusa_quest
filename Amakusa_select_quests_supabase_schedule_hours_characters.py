...
FEATURE_SURVEY_ITEMS = [
    "おすすめクエスト", "全クエスト・検索", "クエストマップ", "写真でのクエストクリア",
...
def save_profile_to_supabase():
    """
    初回の名前・年代を quest_progress の __profile__ 行に保存。
    既存テーブルをそのまま使うのでDBの列追加は不要。
    """
    pid = st.session_state.participant_id.strip()
    if not pid or not supabase_configured():
        return False
...
def _load_profile_from_row(row):
    if not row or not row.get("note"):
        return
    try:
        ...
        if payload.get("nickname"):
            st.session_state.nickname = str(payload.get("nickname")).strip()
        if payload.get("age"):
            st.session_state.profile_age = str(payload.get("age")).strip()
    except Exception:
        pass

def _archive_current_survey_if_needed(pid, new_payload):
    """
    同じ参加者がアンケートを再送信するとき、
    現在の __survey__ の内容を履歴行へコピーしてから更新する。
    これにより過去回答を上書きで失わない。
    """
    if not pid or not supabase_configured():
        return
    ...
    try:
        ...
        old_row = rows[0]
        old_note = old_row.get("note") or ""
        new_note = json.dumps(new_payload, ensure_ascii=False)
        ...
        history_row = {
            ...
            "quest_id": history_id,
            "completed": True,
            "completed_at": old_row.get("completed_at"),
            "favorite": False,
            "note": old_note,
            "photo_uploaded": False,
            "sns_text": "",
        ...
        sb.table("quest_progress").insert(history_row).execute()
    except Exception:
        # 履歴保存失敗だけで本回答送信を止めない
        pass
...
def save_survey_to_quest_progress(payload):
    ...
    # 再回答前の回答を履歴として残す
    _archive_current_survey_if_needed(pid, payload)
        ...
        try:
            ...
            if survey and survey.get("note"):
                try:
                    ...
                    st.session_state.survey_submitted_at = st.session_state.survey_answers.get("submitted_at")
                except Exception:
                    pass
            ...
            for r in rows:
                ...
                if not qid or qid in {APP_STATE_QUEST_ID, SURVEY_QUEST_ID, PROFILE_QUEST_ID} or str(qid).startswith(SURVEY_HISTORY_PREFIX) or not get_quest(qid):
                    continue
...
def render_survey():
    """
    メインアンケート。
    st.form を使わず、スマホでもフォームキー衝突やネスト例外が起きにくい構造にする。
    """
    ...
    if st.session_state.survey_submitted:
        st.success("✅ 回答済みです。内容を変更して再送信できます。")

    ans = dict(st.session_state.survey_answers or {})
    pid_key = re.sub(
        r"[^A-Za-z0-9_-]",
        "_",
        st.session_state.participant_id or "guest"
    )

    gender_opts = ["選択してください", "男性", "女性", "回答しない・その他"]
    region_opts = [
        "選択してください", "天草", "熊本県内（天草外）",
        "九州地方（熊本県外）", "関東地方", "関西地方", "その他"
    ]
    companion_opts = [
        "一人旅", "家族（配偶者・パートナー）", "家族（子ども連れ）",
        "家族（親・その他親族）", "友人・知人", "恋人",
        "旅行ではない（天草在住・日常利用）", "その他"
    ]
    game_opts = ["選択してください", "よくする", "たまにする", "あまりしない", "全くしない"]
    visit_opts = ["選択してください", "初めて", "2回目", "3〜5回目", "6回目以上（リピーター）", "天草在住"]
    change_opts = [
        "選択してください", "1：大きく下がった", "2：やや下がった",
        "3：変わらない", "4：やや高まった", "5：大きく高まった",
        "該当しない（天草在住）"
    ]
    intent_opts = [
        "選択してください", "1：全くそう思わない", "2：あまりそう思わない",
        "3：どちらともいえない", "4：そう思う", "5：とてもそう思う",
        "該当しない（天草在住）"
    ]
    sat_opts = [
        "選択してください", "1：とても不満", "2：やや不満",
        "3：どちらともいえない", "4：満足", "5：とても満足"
    ]
    travel_spend = [
        "回答しない", "0円", "1〜999円", "1,000〜2,999円", "3,000〜4,999円",
        "5,000〜9,999円", "10,000〜19,999円", "20,000〜29,999円",
        "30,000〜49,999円", "50,000円以上", "わからない"
    ]
    quest_spend = [
        "選択してください", "0円", "1〜999円", "1,000〜2,999円",
        "3,000〜4,999円", "5,000〜9,999円", "10,000〜19,999円",
        "20,000〜29,999円", "30,000円以上", "わからない"
    ]
    trigger_opts = [
        "選択してください", "はい", "いいえ", "わからない・覚えていない",
        "該当しない（天草在住・日常利用など）"
    ]
    counter_opts = [
        "選択してください", "利用していたと思う", "おそらく利用していた",
    ...
    # 初回に答えた年代を初期値にする。アンケートで変更も可能。
    current_age = st.session_state.profile_age or ans.get("age", "")
    age = st.selectbox(
        "Q1. 年代",
        AGE_OPTIONS[1:],
        index=max(0, idx(AGE_OPTIONS[1:], current_age)),
        key=f"survey_age_{pid_key}",
    )
    gender = st.selectbox(
        "Q2. 性別（任意）",
        gender_opts,
        index=idx(gender_opts, ans.get("gender", "")),
        key=f"survey_gender_{pid_key}",
    )
    region = st.selectbox(
        "Q3. 居住地域（必須）",
        region_opts,
        index=idx(region_opts, ans.get("region", "")),
        key=f"survey_region_{pid_key}",
    )
    region_other = st.text_input(
        "Q3-2. その他の場合",
        value=ans.get("region_other", ""),
        key=f"survey_region_other_{pid_key}",
    )
    companions = st.multiselect(
        "Q4. 同行者（任意・複数可）",
        companion_opts,
        default=[
            x for x in ans.get("companions", [])
            if x in companion_opts
        ],
        key=f"survey_companions_{pid_key}",
    )
    companion_other = st.text_input(
        "Q4-2. その他の場合",
        value=ans.get("companion_other", ""),
        key=f"survey_companion_other_{pid_key}",
    )
    gaming = st.selectbox(
        "Q5. 普段ゲームをしますか？（任意）",
        game_opts,
        index=idx(game_opts, ans.get("gaming", "")),
        key=f"survey_gaming_{pid_key}",
    )
    visits = st.selectbox(
        "Q6. 天草への訪問回数（必須）",
        visit_opts,
        index=idx(visit_opts, ans.get("visits", "")),
        key=f"survey_visits_{pid_key}",
    )
    ...
    with a:
        lodging = st.selectbox(
            "Q7-1. 宿泊費",
            travel_spend,
            index=idx(travel_spend, ans.get("trip_spend_lodging", "回答しない")),
            key=f"survey_lodging_{pid_key}",
        )
        food = st.selectbox(
            "Q7-2. 飲食費",
            travel_spend,
            index=idx(travel_spend, ans.get("trip_spend_food", "回答しない")),
            key=f"survey_food_{pid_key}",
        )
    with b:
        souvenir = st.selectbox(
            "Q7-3. お土産・買い物代",
            travel_spend,
            index=idx(travel_spend, ans.get("trip_spend_souvenir", "回答しない")),
            key=f"survey_souvenir_{pid_key}",
        )
        experience = st.selectbox(
            "Q7-4. 体験・施設入場料",
            travel_spend,
            index=idx(travel_spend, ans.get("trip_spend_experience", "回答しない")),
            key=f"survey_experience_{pid_key}",
        )

    st.markdown("### ■ クエスト掲載店・施設への送客効果")
    listing_spend = st.selectbox(
        "Q8. クエスト掲載店・有料施設で使った金額合計（必須・1人あたり）",
        quest_spend,
        index=idx(quest_spend, ans.get("quest_listing_spend", "")),
        key=f"survey_listing_spend_{pid_key}",
    )
    triggered = st.selectbox(
        "Q9. アプリ・クエストがきっかけで実際に行ったスポットはありましたか？（必須）",
        trigger_opts,
        index=idx(trigger_opts, ans.get("app_triggered_visit", "")),
        key=f"survey_triggered_{pid_key}",
    )
    triggered_places = st.multiselect(
        "Q9-2. 『はい』の方：アプリがきっかけで訪れたスポット",
        places,
        default=[
            x for x in ans.get("app_triggered_places", [])
            if x in places
        ],
        key=f"survey_triggered_places_{pid_key}",
    )
    triggered_spend = st.selectbox(
        "Q9-3. 『はい』の方：その掲載店・有料施設で使った金額合計",
        quest_spend,
        index=idx(quest_spend, ans.get("app_triggered_spend", "")),
        key=f"survey_triggered_spend_{pid_key}",
    )
    counter = st.selectbox(
        "Q10. 『はい』の方：このアプリがなくても、そのスポットを訪問・利用していたと思いますか？",
        counter_opts,
        index=idx(counter_opts, ans.get("counterfactual_visit", "")),
        key=f"survey_counter_{pid_key}",
    )

    st.markdown("### ■ アプリ機能評価")
    ratings = {}
    saved = ans.get("feature_ratings", {}) or {}
    for i, feature in enumerate(FEATURE_SURVEY_ITEMS, 11):
        ratings[feature] = st.radio(
            f"Q{i}. {feature}",
            FEATURE_RATING_OPTIONS,
            index=idx(
                FEATURE_RATING_OPTIONS,
                saved.get(feature, "使っていない")
            ),
            horizontal=True,
            key=f"survey_rating_{pid_key}_{i}",
        )
    ...
    revisit_change = st.selectbox(
        f"Q{qn}. アプリ利用で『また来たい』気持ちは高まりましたか？（必須）",
        change_opts,
        index=idx(change_opts, ans.get("revisit_change", "")),
        key=f"survey_revisit_change_{pid_key}",
    )
    revisit_intent = st.selectbox(
        f"Q{qn+1}. 今後1年以内に天草を再訪したいですか？（必須）",
        intent_opts,
        index=idx(intent_opts, ans.get("revisit_intent", "")),
        key=f"survey_revisit_intent_{pid_key}",
    )
    reuse_intent = st.selectbox(
        f"Q{qn+2}. 次回もこのアプリを使いたいですか？（必須）",
        intent_opts,
        index=idx(intent_opts, ans.get("reuse_intent", "")),
        key=f"survey_reuse_intent_{pid_key}",
    )
    satisfaction = st.selectbox(
        f"Q{qn+3}. アプリ全体の満足度（必須）",
        sat_opts,
        index=idx(sat_opts, ans.get("overall_satisfaction", "")),
        key=f"survey_satisfaction_{pid_key}",
    )

    good = st.text_area(
        f"Q{qn+4}. 良かった点（任意）",
        value=ans.get("good_points", ""),
        key=f"survey_good_{pid_key}",
    )
    improve = st.text_area(
        f"Q{qn+5}. 改善点（任意）",
        value=ans.get("improvement_points", ""),
        key=f"survey_improve_{pid_key}",
    )
    request = st.text_area(
        f"Q{qn+6}. 追加してほしい機能（任意）",
        value=ans.get("requested_features", ""),
        key=f"survey_request_{pid_key}",
    )
    ...
    missing = []
    checks = [
        (region == "選択してください", "居住地域"),
        (visits == "選択してください", "天草訪問回数"),
        (listing_spend == "選択してください", "掲載店・施設での消費額"),
        (triggered == "選択してください", "アプリによる来訪の有無"),
        (revisit_change == "選択してください", "再訪意欲の変化"),
        (revisit_intent == "選択してください", "再訪意向"),
        (reuse_intent == "選択してください", "再利用意向"),
        (satisfaction == "選択してください", "満足度"),
    ]
    ...
    if region == "その他" and not region_other.strip():
        missing.append("その他の居住地域")

    if "一人旅" in companions and len(companions) > 1:
        missing.append("同行者（一人旅と他項目は併用不可）")

    if triggered == "はい":
        if not triggered_places:
            missing.append("アプリがきっかけで訪れたスポット")
        if triggered_spend == "選択してください":
            missing.append("アプリ起点の消費額")
        if counter == "選択してください":
            missing.append("アプリがなかった場合の訪問意向")

    if missing:
        st.error("未回答の必須項目があります：" + "、".join(missing))
        return
    ...
    payload = {
        ...
        "gaming": gaming,
        "visits": visits,
        "trip_spend_lodging": lodging,
        "trip_spend_food": food,
        "trip_spend_souvenir": souvenir,
        "trip_spend_experience": experience,
        "quest_listing_spend": listing_spend,
        "app_triggered_visit": triggered,
        "app_triggered_places": triggered_places if triggered == "はい" else [],
        "app_triggered_spend": triggered_spend if triggered == "はい" else "該当なし",
        "counterfactual_visit": counter if triggered == "はい" else "該当なし",
        "feature_ratings": ratings,
        "revisit_change": revisit_change,
        "revisit_intent": revisit_intent,
        "reuse_intent": reuse_intent,
        "overall_satisfaction": satisfaction,
        "good_points": good.strip(),
        "improvement_points": improve.strip(),
        "requested_features": request.strip(),
        "submitted_at": datetime.now(timezone.utc).isoformat(),
    }
    ...
    err = None
    try:
        saved_ok = save_survey_to_quest_progress(payload)
    except Exception as e:
        err = e

    # app_state も保存。ただし survey 保存結果とは独立させる
    try:
        save_user_data()
    except Exception:
        pass
    ...
    if saved_ok:
        st.success("✅ ご回答ありがとうございました。Supabaseへの保存を確認しました。")
    elif err:
        st.error(
...
# =====================================================================
# 管理者モード：初回年代 / 終了後アンケート / 全体アンケートを一元確認
# =====================================================================

def _json_note(row):
    try:
        data = json.loads(row.get("note") or "{}")
...
def build_admin_profile_df(rows):
    """
    新しい __profile__ を優先し、
    既存参加者は __app_state__ / __survey__ から年代・名前を復元する。
    これにより過去データも表示対象にする。
    """
    by_pid = {}
    ...
    for row in rows:
        ...
        if qid == APP_STATE_QUEST_ID:
            ...
            rec["最終更新"] = data.get("survey_submitted_at") or row.get("completed_at") or ""
    ...
    for row in rows:
        ...
        if not pid:
            continue
        if row.get("quest_id") == SURVEY_QUEST_ID:
            data = _json_note(row)
            rec = ensure(pid)
    ...
    for row in rows:
        ...
        if not pid:
            continue
        if row.get("quest_id") == PROFILE_QUEST_ID:
            data = _json_note(row)
            rec = ensure(pid)
            if data.get("nickname"):
                rec["ニックネーム"] = data.get("nickname")
            if data.get("age"):
                rec["初回年代"] = data.get("age")
                rec["年代の取得元"] = "初回入力"
            rec["最終更新"] = data.get("updated_at") or rec["最終更新"]

    return pd.DataFrame(list(by_pid.values()))

def build_admin_feedback_df(rows):
    records = []
    for row in rows:
        qid = str(row.get("quest_id", ""))
        if not qid.startswith(QUEST_FEEDBACK_PREFIX):
        ...
        actual_qid = qid[len(QUEST_FEEDBACK_PREFIX):]
        q = get_quest(actual_qid)
        data = _json_note(row)

        records.append({
            "participant_id": row.get("participant_id", ""),
            "場所": data.get("linked_name") or q.get("linked_name", ""),
            "クエスト": data.get("quest_name") or q.get("quest_name", ""),
            "Q1 楽しさ★": data.get("fun_rating", ""),
            "Q2 キャラ獲得が訪問動機★": data.get("character_motivation_rating", ""),
            "Q3 改善点": data.get("improvement", ""),
            "回答日時": data.get("submitted_at") or row.get("completed_at") or "",
        })

    return pd.DataFrame(records)

def _survey_admin_record(row, record_type="現在回答"):
    data = _json_note(row)
    features = data.get("feature_ratings", {}) or {}
    ...
    rec = {
        ...
        "年代": data.get("age", ""),
        "性別": data.get("gender", ""),
        "居住地域": data.get("region", ""),
        "居住地域その他": data.get("region_other", ""),
        "同行者": " / ".join(data.get("companions", []) or []),
        "ゲーム頻度": data.get("gaming", ""),
        "天草訪問回数": data.get("visits", ""),
        "宿泊費": data.get("trip_spend_lodging", ""),
        "飲食費": data.get("trip_spend_food", ""),
        "お土産・買い物代": data.get("trip_spend_souvenir", ""),
        "体験・施設入場料": data.get("trip_spend_experience", ""),
        "掲載店・施設消費額": data.get("quest_listing_spend", ""),
        "アプリ起点訪問": data.get("app_triggered_visit", ""),
        "アプリ起点スポット": " / ".join(data.get("app_triggered_places", []) or []),
        "アプリ起点消費額": data.get("app_triggered_spend", ""),
        "アプリなしでも訪問したか": data.get("counterfactual_visit", ""),
        "再訪意欲の変化": data.get("revisit_change", ""),
        "1年以内の再訪意向": data.get("revisit_intent", ""),
        "次回アプリ利用意向": data.get("reuse_intent", ""),
        "アプリ全体満足度": data.get("overall_satisfaction", ""),
        "良かった点": data.get("good_points", ""),
        "改善点": data.get("improvement_points", ""),
        "追加希望機能": data.get("requested_features", ""),
    }

    for feature, value in features.items():
        rec[f"機能評価｜{feature}"] = value

    # 古い回答の未知フィールドも失わないため、生JSONも保持・表示
    rec["元回答JSON"] = json.dumps(data, ensure_ascii=False)

    return rec

def build_admin_survey_df(rows):
    records = []
    current_pids = set()

    # 1) 現在の正式な __survey__ 回答
    for row in rows:
        if row.get("quest_id") == SURVEY_QUEST_ID:
            pid = str(row.get("participant_id", "")).strip()
            current_pids.add(pid)
            records.append(_survey_admin_record(row, "現在回答"))

    # 2) 過去バージョンで survey_answers が __app_state__ 内だけに
    #    保存されていた参加者も、管理者画面では回答として復元して表示する。
    #    __survey__ が既にある参加者は重複させない。
    for row in rows:
        if row.get("quest_id") != APP_STATE_QUEST_ID:
            continue
        ...
        app_data = _json_note(row)
        legacy_survey = app_data.get("survey_answers") or {}

        if not isinstance(legacy_survey, dict) or not legacy_survey:
            continue

        # _survey_admin_record が読める形の仮想rowを作るだけで、
        # Supabase上の元データ自体は変更・削除しない。
        virtual_row = {
            "participant_id": pid,
            "quest_id": SURVEY_QUEST_ID,
            "completed_at": legacy_survey.get("submitted_at"),
            "note": json.dumps(legacy_survey, ensure_ascii=False),
        }
        records.append(
            _survey_admin_record(
                virtual_row,
                "既存app_stateから復元"
            )
        )
    ...
    for row in rows:
        qid = str(row.get("quest_id", ""))
        if qid.startswith(SURVEY_HISTORY_PREFIX):
            records.append(_survey_admin_record(row, "過去回答（履歴）"))

    return pd.DataFrame(records)

def build_admin_quest_df(rows):
    special = {
        APP_STATE_QUEST_ID,
        SURVEY_QUEST_ID,
        PROFILE_QUEST_ID,
    ...
    for row in rows:
        ...
        if (
            not qid
            or qid in special
            or qid.startswith(QUEST_FEEDBACK_PREFIX)
            or qid.startswith(SURVEY_HISTORY_PREFIX)
        ):
            continue
        ...
        records.append({
            "participant_id": row.get("participant_id", ""),
            "quest_id": qid,
            "場所": q.get("linked_name", ""),
            "クエスト": q.get("quest_name", ""),
            "クリア": bool(row.get("completed")),
            "クリア日時": row.get("completed_at") or "",
            "写真登録": bool(row.get("photo_uploaded")),
            "旅日記": row.get("note") or "",
...
def _download_df_button(df, label, filename, key):
...
def render_admin_mode():
    with st.expander("🔐 管理者モード｜実証結果を見る", expanded=False):
        admin_password = safe_secret("ADMIN_PASSWORD", "")

        if not admin_password:
            st.warning(
                "管理者データを参加者に公開しないため、"
                "Streamlit Secrets に ADMIN_PASSWORD を設定してください。"
            )
            st.code('ADMIN_PASSWORD = "任意の管理者パスワード"')
            return
        ...
        survey_df = build_admin_survey_df(rows)
        quest_df = build_admin_quest_df(rows)

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("参加者", len(profile_df) if not profile_df.empty else 0)
        ...
        with t1:
            ...
            if profile_df.empty:
                st.info("参加者データはまだありません。")
            else:
                st.dataframe(profile_df, use_container_width=True, hide_index=True)
                _download_df_button(
                    profile_df,
                    "CSVをダウンロード",
                    "amakusa_profiles.csv",
                    "download_profiles",
                )

        with t2:
            st.subheader("クエスト終了後の3問アンケート")
            if feedback_df.empty:
                st.info("終了後アンケートの回答はまだありません。")
            else:
                st.dataframe(feedback_df, use_container_width=True, hide_index=True)

                if "Q1 楽しさ★" in feedback_df.columns:
                    q1 = pd.to_numeric(feedback_df["Q1 楽しさ★"], errors="coerce")
                    q2 = pd.to_numeric(feedback_df["Q2 キャラ獲得が訪問動機★"], errors="coerce")
                    a, b = st.columns(2)
                    a.metric("平均｜楽しさ", f"{q1.mean():.2f} ★" if q1.notna().any() else "-")
                    b.metric("平均｜キャラの訪問動機", f"{q2.mean():.2f} ★" if q2.notna().any() else "-")

                _download_df_button(
                    feedback_df,
                    "CSVをダウンロード",
                    "amakusa_quest_feedback.csv",
                    "download_feedback",
                )

        with t3:
            st.subheader("既存＋新規の全体アンケート")
            st.caption(
                "既にSupabaseへ保存されている __survey__ の回答は削除・初期化せず、そのまま読み込みます。"
                "今後、同一参加者が再回答した場合は更新前の内容を履歴として残します。"
            )
            if survey_df.empty:
                st.info("Supabase上に読み込める全体アンケート回答がありません。")
            else:
                st.dataframe(survey_df, use_container_width=True, hide_index=True)
                _download_df_button(
                    survey_df,
                    "CSVをダウンロード",
                    "amakusa_survey_all.csv",
                    "download_survey",
                )

        with t4:
            st.subheader("参加者ごとのクエスト記録")
            if quest_df.empty:
                st.info("クエスト記録はまだありません。")
            else:
                st.dataframe(quest_df, use_container_width=True, hide_index=True)
                _download_df_button(
                    quest_df,
                    "CSVをダウンロード",
                    "amakusa_quest_progress.csv",
                    "download_quest_progress",
                )
...
if (
    ...
    st.markdown(
        '<div class="name-hint">この情報は実証実験の集計と、あなたの旅の記録管理に使用します。</div>',
        unsafe_allow_html=True,
    )
    ...
    start_button = st.button(
        ...
        type="primary",
        use_container_width=True,
        key="initial_start_button",
    )

    if start_button:
        missing = []
        if not first_name:
            missing.append("参加者のお名前")
        if first_age == "選択してください":
            missing.append("年代")
        ...
        if missing:
            ...
            if supabase_configured() and not profile_saved:
                st.warning(
                    "名前・年代はこの画面には保存しましたが、"
                    "Supabaseへのプロフィール保存を確認できませんでした。"
                )
            st.rerun()
...
with st.expander("👤 参加者情報を変更する"):
    ...
    new_age = st.selectbox(
        "年代",
        AGE_OPTIONS[1:],
        index=max(0, idx(AGE_OPTIONS[1:], st.session_state.profile_age)),
